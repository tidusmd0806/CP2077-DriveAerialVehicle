...
--- Initialize and attach route planning methods to AV class.
---@param AV table
function Navigation:Init(AV)
...
--- Attach route planning methods to AV class.
---@param AV table
function Navigation:AttachTo(AV)
	...
	---@return integer
	function AV:GetObstacleCellPriority(value)
		if value == true then
			return 3
	...
	function AV:SetObstacleCellNoDirty(key, value)
	...
	---@return boolean success
	function AV:IntegrateObstacleMapDiff()
		if next(self.obstacle_map) == nil then
			self.log_obj:Record(LogLevel.Warning, "IntegrateObstacleMapDiff skipped: in-memory map is empty")
			return false
		end
	...
	function AV:ComputeLocalAvoidanceDirection(current_pos, dest_dir_vec, current_time)
		...
		if self.local_avoidance_net_check_dist == nil then
		...
		if self.local_avoidance_stuck_timer >= self.local_avoidance_stuck_threshold then
